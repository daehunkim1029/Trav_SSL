...
class OccFusion(Base3DSegmentor):
    ...
    def loss(self,batch_inputs, batch_data_samples):
        ...
        if X==256:
            voxels_lvl0 = self.multiscale_supervision(batch_inputs['occ_semantickitti_masked'],[1,1,1],np.array([len(batch_data_samples),256,256,64],dtype=np.int32))
            voxels_lvl1 = self.multiscale_supervision(batch_inputs['occ_semantickitti_masked'],[2,2,2],np.array([len(batch_data_samples),128,128,32],dtype=np.int32))
            voxels_lvl2 = self.multiscale_supervision(batch_inputs['occ_semantickitti_masked'],[4,4,4],np.array([len(batch_data_samples),64,64,16],dtype=np.int32))
            voxels_lvl3 = self.multiscale_supervision(batch_inputs['occ_semantickitti_masked'],[8,8,8],np.array([len(batch_data_samples),32,32,8],dtype=np.int32))
        elif self.occ3d:
