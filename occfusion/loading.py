...
class LoadSemanticKITTI_Occupancy(BaseTransform):
    def get_remap_lut(self, label_map):
        """
        label_map이 4개 클래스에 대해 identity mapping을 지정한다고 가정합니다.
        예: {0: 0, 1: 1, 2: 2, 3: 3, 255: 255}  
        (255는 invalid/빈 공간)
        """
    ...
    def transform(self, result: dict) -> dict:
        ...
        gt = np.fromfile(result['pts_semantic_mask_path'], dtype=np.uint16).astype(np.float32)
        ...
        # reshape
        gt_masked = gt.reshape([256, 256, 64] ) 

        gt_masked = torch.from_numpy(gt_masked)

        # 여기서 255는 invalid이므로 제외
        # => 0~3 만 사용
        # => 0도 유효하다고 하셨으므로, 0,1,2,3만 남기고 255는 제외
